...
def get_args() -> DDIMInversionArguments:
    ...
    parser.add_argument(
        "--output_path", type=str, default="output", help="Dir of the output videos"
    )
    parser.add_argument(
...
def get_video_frames(
    ...
    with decord.bridge.use_torch():
        ...
        if target > n:
            last = frames[-1]  
            pad = last.unsqueeze(0).expand(target - n, -1, -1, -1).clone()
...
def ddim_inversion(
    ...
    # export_latents_to_video(pipeline, video_latents, inverse_video_path, fps)
    # export_latents_to_video(pipeline, recon_latents[-1], recon_video_path, fps)
    
    inverse_latent_path = output_path
    inverse_latents_all = torch.cat([video_latents.unsqueeze(0), inverse_latents], dim=0)
    torch.save(inverse_latents_all, inverse_latent_path)
